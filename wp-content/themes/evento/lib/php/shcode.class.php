	...
	class shcode{
        static function contact( $attr , $content ){
            ...
            if( strlen( options::get_value( 'social' , 'google_map' ) ) ){
                ...
                $result .= '<script src="' . get_template_directory_uri() . '/lib/js/actions.js" type="text/javascript"></script>';
        		...
        		if($attr[ 'conf_id' ] != 'all_conferences' && is_numeric($conf_id) ){
        			...
        			foreach ($posts as $post) {
						...
						if(!isset($conf_settings['archive']) || (isset($conf_settings['archive']) && $conf_settings['archive'] == 'no')){
							...
							$result .= '<h3 class="entry-title"><a href="'. get_permalink($post -> ID ) .'" title="'.get_the_title($post -> ID).'">'.get_the_title($post -> ID).'</a></h3>';
							if($post -> post_excerpt != '' ){
								$result .= '<p>'.get_the_excerpt_here($post -> ID).'</p>';
							}else{
								$result .= '<p>'.strip_tags(mb_substr($post -> post_content,0,EXCERPT_CHAR_LEN) ).'</p>';
							}	
							/* read more button */
        ...
        static function add_exhibitors( $attr ){
            ...
            if( isset( $attr[ 'conf_id' ] ) ){
                ...
                if( isset( $attr[ 'order_by' ] ) && $attr[ 'order_by' ] == 'alpha'){
                    /*BOF Sort exhibitors in alphabetical order*/  
                    $temp_exhibitors_info = array();
                    foreach( $exhibitors as $exhibitor ){ 
                        $post1 = get_post( $exhibitor['idrecord'] );   
                        $temp_exhibitors_info[$exhibitor['idrecord']] = $post1 -> post_title;  
                    } 
                    
                    asort($temp_exhibitors_info); /*sort exhibitors*/
                    
                    
                    foreach( $temp_exhibitors_info as $exhibitor ){ 
                        $exhibitors_temp[] = array('idrecord' => array_search($exhibitor, $temp_exhibitors_info)); 
                    }
                    $exhibitors = $exhibitors_temp;
                    /*EOF Sort exhibitors in alphabetical order*/
                }
                ...
                if( is_array( $exhibitors ) && count( $exhibitors ) ){
                    ...
                    foreach( $exhibitors as $exhibitor ){
                        ...
                        $result .= '</h4>';
                        if($post -> post_excerpt != '' ){
	        				$result .= '<span  class="exc">' . get_the_excerpt_here($post -> ID) . '</span>';
	        			}else{
	        				$result .= '<span  class="exc">' . strip_tags(mb_substr($post -> post_content,0,EXCERPT_CHAR_LEN) ). '</span>';  
	        			}
        		...
        		if(isset( $attr[ 'display_mode' ] ) && $attr[ 'display_mode' ] == 'animated'){
	        		...
	        		if( is_array( $testimonials ) && count( $testimonials ) ){
	                    ...
	                    foreach( $testimonials as $testimonial ){
							...
							$result .= '<span class="testimonial_author_name"> '.$testimonial_info['name'].'</span>';
							$result .= $author_title;
	                        $result .= '</h4>';
	                        
	                        $result .= '<span  class="exc">' . $post -> post_content . '</span>';
            ...
            if( is_array( $speakers ) && count( $speakers ) ){ 
                ...
                foreach( $speakers as $speaker ){ 
					...
					if($post -> post_excerpt != '' ){
						...
						$result .= '<span  class="exc">' . strip_tags(mb_substr(strip_shortcodes($post -> post_content),0,EXCERPT_CHAR_LEN) ). $continue . '</span>';  
					}
                    $result .= '<p class="button readmore hover margin15">';
                    $result .= '<a href="' . get_permalink( $post -> ID ) . '">' . __( 'Continue reading' , 'cosmotheme' ) . '<span>&nbsp;</span></a>';
                    $result .= '</p>';
                    ...
                    foreach( $new_event as $date => $events ){
                        ...
                        foreach( $events as $index => $event ){
							...
							/*if ($prev_iteration_time != date(get_option( time_format ),strtotime($start_time) ).' - '. date(get_option( time_format ),strtotime($end_time) )){*/
								...
								$result .= '</span>';
							/*}*/	
                            if ( isset($event['title2']) && $event['title2'] ==""){
                                $result .= '<span class="event">' . $event['title'] . '</span>';
                            } else if ( isset($event['title2']) ){
