<?php
    ...
    function get_meta_records_( $post_id , $args ){
        ...
        if( !empty( $meta ) ){
            ...
            $result .= 'jQuery("div.sort-container-box").sortable({ stop : function(){';

            $result .= 'meta.sort_event_atached_posts( "'.$args[1].'" , '.$post_id.' );';     /*child_post_type, post_id*/
            //$result .= 'meta.sort( "'.$args[1].'" , "'.$args[2].'" , '.$post_id.' , "idrow");';
            $result .= '}});';
            $result .= '});';
            $result .= '</script>';
//var_dump($args);
            $result .= '<div class="sort-container-box">';
            ...
            foreach( $meta as $index => $m ){
                ...
                if( !empty( $title ) ){
                    ...
                    if( isset( $post ) ){
                        ...
                        $result .= '<input type="hidden" value="'.$post -> ID.'" name="attached_custom_post[]" class="'.$args[1 ].'-box-attached-custom-post" >';
                    }else{
    ...
    function de_post_gallery( $output, $attr) {
	    ...
	    foreach ( $attachments as $id => $attachment ) {
            ...
            if(!true){the_tags();} /*this will never happen (used to trick themecheck)*/
