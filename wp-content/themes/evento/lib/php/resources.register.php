<?php
    ...
    $labels['conference']['tag'] = array(
        ...
        'singular_name' => _x( 'Tags', 'taxonomy singular name','cosmotheme' ),
        ...
        'actions' => array(
            ...
            1 => array( 
                ...
                'args' => array( 
                    ...
                    'index' => '' , 
                    'data' => array( 
                        'input' =>  "['conference-program-title', 'conference-program-title2']" , 
                        'select' =>  "['conference-program-end_min' , 'conference-program-end_hour' , 'conference-program-end_y' , 'conference-program-end_m' , 'conference-program-end_d' , 'conference-program-start_min' , 'conference-program-start_hour' , 'conference-program-start_y' , 'conference-program-start_m' , 'conference-program-start_d']" , 
                        'textarea' => "'conference-program-description'" 
                        ) , 
                    'selector' => 'div#conference_program div.inside div#box_conference_program' 
                    ) 
                ),
    ...
    $form['conference']['program']['title2'] = array( 'type' => 'st--m-text' , 'label' => __( 'Event link' , 'cosmotheme' ), 'hint'=>__("This is title's link, e.g. http://google.com","cosmotheme") );
    ...
    if ( isset( $_GET['post_type'] ) ){ // used on creation, for different post types, we will have different default setting for layout
        $def_layout_type = options::get_value( 'layout' , $_GET['post_type'] );
    } elseif (isset($_GET['post']) ) { // used when editing a post
        $meta = meta::get_meta( (int) $_GET['post'] , 'layout' ); //we read the layout saved in post meta
        if ( isset($meta['type'])) {
            $def_layout_type = $meta['type'];
        } else {
            $def_layout_type = '';
        }

    } else {
        $def_layout_type = options::get_value( 'layout' , 'single' ); //new standard post creation, the layout is taken from default settings
    }
    

    $form['conference']['layout']['type']               = array( 'type' => 'sh--select' , 'label' =>  __( 'Select layout type' , 'cosmotheme' ) , 'ivalue' => $def_layout_type, 'value' => array( 'right' => __( 'Right Sidebar'  , 'cosmotheme' ) , 'left' => __( 'Left Sidebar' , 'cosmotheme' ) , 'full' => __( 'Full Width' , 'cosmotheme' )  ) , 'action' => "act.select( '#post_layout' , { 'full' : '.sidebar_list' } , 'hs_');" , 'id' => 'post_layout' );
    ...
    */
    ...
    $res['sponsor']['labels'] = array(
        ...
        'singular_name' => _x('Sponsor', 'post type singular name','cosmotheme'),
        'add_new' => _x('Add New', 'Sponsor','cosmotheme'),
    ...
    $res['presentation']['labels'] = array(
        ...
        'singular_name' => _x('Presentation', 'post type singular name','cosmotheme'),
        'add_new' => _x('Add New', 'Presentation','cosmotheme'),
