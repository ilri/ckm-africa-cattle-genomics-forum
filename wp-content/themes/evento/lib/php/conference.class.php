<?php
    class conference {
        ...
        static function speakers( $post_id , $limit = 10, $category=false ){
            /* get presentations */
            ...
            if( !empty( $presentations_speakers ) ){
                ...
                foreach( $speakers as $index => $speaker ){
                    ...
                    $result .= '<a href="' . get_permalink( $post -> ID  ). '" class="readmore">' . $post -> post_title . '<span class="mosaic-overlay">&nbsp;</span></a>';
        ...
        static function get_conferences( $type = 'latest' , $nr=2 , $show_images = true, $category=false ){
        	$result ='';	
				...
				foreach ($posts as $post) {
					...
					if( ($type == "latest") || ( (!isset($conf_settings['archive']) || (isset($conf_settings['archive']) && $conf_settings['archive'] == 'no')) && $conf_count<$nr) ){
						...
						$result .= '<a href="' . get_permalink( $post -> ID  ). '" class="readmore '.$additional_class.'">' . $post -> post_title . '<span class="mosaic-overlay">&nbsp;</span></a>';
        ...
        static function get_speakers( $post_id ){
            /* get presentations */
            $presentations = meta::get_meta( $post_id , 'presentation' );
            foreach( $presentations as $presentation ){
                $post = get_post( $presentation['idrecord'] );
        ...
        static function presentation_speakers( $post_id , $limit = 10 ){
            ...
            foreach( $speakers as $index => $speaker ){
                ...
                $result .= '<a href="' . get_permalink( $post -> ID  ). '">' . $img . '<span class="info">' . $post -> post_title . ' <i>' . $study . ' '. $university .'</i></span></a>';
                $result .= '</p>';
        ...
        static function presentations( $post_id , $limit = 0, $category=false){
            $presentations = meta::get_meta( $post_id , 'presentation' );
            $result = '';
            if( count( $presentations ) ){
                ...
                foreach( $presentations as $index => $presentation ){
                    ...
                    /* title conference */
                    $result .= '<li><a href="' . get_permalink( $post -> ID ) . '" class="readmore">' . $post -> post_title . '<span class="mosaic-overlay">&nbsp;</span></a>';
                    /* list speakers */
                    $result .= '<ul>';
        ...
        static function get_testimonials( $post_id , $limit = 0 ){
            ...
            if( count( $testimonials ) ){
            	...
            	$result .= '<span class="actions">
								<a class="prev"><i class="b-pager__arr">&larr;</i> Previous</a>
								<a class="next">Next <i class="b-pager__arr">&rarr;</i></a>
							</span>';
            	}
            	$result .= '</div>';
        ...
        static function exhibitors( $post_id , $limit = 0 , $excerpt = true , $category=false ){
            ...
            if( count( $exhibitors ) ){
                ...
                foreach( $exhibitors as $index => $exhibitor ){
                    ...
                    $result .= '<a href="' . get_permalink( $post -> ID  ). '" class="readmore">' . $post -> post_title . '<span class="mosaic-overlay">&nbsp;</span></a>';
                    if( $excerpt ){
                        $result .= '<span class="desc">' . mb_substr( $post -> post_excerpt , 0 , 250 ) . '</span>';
                    }
                    $result .= '</h4>';
        ...
        static function sponsors( $post_id , $limit = 0 , $excerpt = true, $category=false ){
            ...
            if( count( $sponsors ) ){
                ...
                foreach( $sponsors as $index => $sponsor ){
                    ...
                    $result .= '<a href="' . get_permalink( $post -> ID  ). '" class="readmore">' . $post -> post_title . '<span class="mosaic-overlay">&nbsp;</span></a>';
                    if( $excerpt ){
                        $result .= '<span class="desc">' . $post -> post_excerpt . '</span>';
                    }
                    $result .= '</h4>';
                    $result .= '</li>';
                    if( $limit != 0 && $index == $limit - 1 ){
        ...
        static function get_sponsors_animated($post_id , $total_limit = 0 , $visible_limit = 1, $show_name = false ){
        ...
        static function program ( $post_id , $limit = 0 , $description = false, $class_ul = 'ul' ){
            ...
            if( count( $events ) ){ 
                ...
                foreach( $new_event as $date => $events ){
                    ...
                    $day = date_i18n( get_option('date_format' ) , $date );
                    $result .= '<li>' . $day ;
                    $result .= '<ul>';
                    ...
                    foreach( $events as $index => $event ){
						...
						/*if ($prev_iteration_time != date(get_option( time_format ),strtotime($start_time) ).' - '. date(get_option( time_format ),strtotime($end_time) )){*/
							...
							$result .= date_i18n(get_option( 'time_format' ),strtotime($start_time) ).' - '. date_i18n(get_option( 'time_format' ),strtotime($end_time) );
							$prev_iteration_time = date_i18n(get_option( 'time_format' ),strtotime($start_time) ).' - '. date_i18n(get_option( 'time_format' ),strtotime($end_time) );
							$result .= '</span>';
						/*}*/	
                        ...
                        if( $description ){
                            $result .= '<span class="event">' . $event['description'] . '</span>';
                        }
        ...
        static function guests( $post_id , $limit = 0 , $class_ul = 'ul'){
            ...
            if( count( $guests ) ){
                ...
                foreach( $guests as $index => $guest ){
                    $result .= '<li>' . get_the_author_meta( 'first_name' , $guest['idrecord'] )  . ' ' . get_the_author_meta( 'last_name' , $guest['idrecord'] )  . ' ( ' . get_the_author_meta( 'nickname' , $guest['idrecord'] ) . ' )</li>';
        ...
        static function get_pricing($post_id = 0){ 
        	$result = '';	
